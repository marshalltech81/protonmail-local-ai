...
otherwise-sync test functions, matching ``test_ollama.py`` and
``test_imap.py``.
"""
...
def _handler(fake_server, fake_ollama, db):
    register_search_tools(fake_server, db, fake_ollama)
...
class TestModeValidation:
    def test_invalid_mode_returns_validation_message(self, fake_server, fake_ollama, seeded_db):
    ...
    def test_all_valid_modes_are_accepted(self, fake_server, fake_ollama, seeded_db, mode):
        handler = _handler(fake_server, fake_ollama, seeded_db)
...
class TestLimitClamping:
    def test_above_ceiling_is_clamped(self, fake_server, fake_ollama, seeded_db):
    ...
    def test_below_floor_is_clamped(self, fake_server, fake_ollama, seeded_db):
        seen: dict[str, int] = {}
        original = seeded_db.hybrid_search
...
class TestModeRouting:
    def test_keyword_mode_skips_embed_call(self, fake_server, fake_ollama, seeded_db):
        handler = _handler(fake_server, fake_ollama, seeded_db)
        asyncio.run(handler(query="invoice", mode="keyword"))
        # keyword mode must not pay for an embedding the db would ignore.
        assert fake_ollama.embed_calls == []

    def test_semantic_mode_embeds_once(self, fake_server, fake_ollama, seeded_db):
        handler = _handler(fake_server, fake_ollama, seeded_db)
        asyncio.run(handler(query="lunch", mode="semantic"))
        assert fake_ollama.embed_calls == ["lunch"]

    def test_hybrid_mode_embeds_once(self, fake_server, fake_ollama, seeded_db):
        handler = _handler(fake_server, fake_ollama, seeded_db)
        asyncio.run(handler(query="meeting", mode="hybrid"))
        assert fake_ollama.embed_calls == ["meeting"]


class TestFilterForwarding:
    def test_keyword_mode_forwards_all_filters(self, fake_server, fake_ollama, seeded_db):
...
class TestResultFormatting:
    def test_empty_results_returns_no_results_message(self, fake_server, fake_ollama, seeded_db):
        handler = _handler(fake_server, fake_ollama, seeded_db)
        out = asyncio.run(handler(query="zxqwzxqw", mode="keyword"))
        assert "No results found" in _text(out)

    def test_formatted_result_includes_key_fields(self, fake_server, fake_ollama, seeded_db):
        handler = _handler(fake_server, fake_ollama, seeded_db)
    ...
    def test_result_count_header_matches_results(self, fake_server, fake_ollama, seeded_db):
        handler = _handler(fake_server, fake_ollama, seeded_db)
...
class TestErrorPath:
    def test_db_exception_returns_error_text(self, fake_server, fake_ollama, seeded_db):
        def boom(**_kwargs):
