...
- fix the first-run setup flow so that `make first-run` cannot fail with "secret file not found" before the Bridge CLI appears; Docker Compose validates all declared secret file paths at parse time even when the service using the secret is not being started, so if `.secrets/bridge_pass.txt` does not exist when `make first-run` runs, compose exits before showing the Bridge CLI; the CI workflow avoids this by creating a placeholder file before any compose command; `README.md` and `docs/setup.md` both have this gap — neither instructs the operator to create a placeholder before running `make first-run`; fix by having `make first-run` create a placeholder `.secrets/bridge_pass.txt` automatically if it does not exist (preferred, self-healing), or update both `README.md` and `docs/setup.md` to add the placeholder creation step
...
- add network egress isolation for `app-net`: in `LLM_MODE=local` the indexer, Ollama, and mcp-server containers have no legitimate reason to reach the internet; consider marking `app-net` as `internal: true` in `docker-compose.yml` so a compromised container on `app-net` cannot exfiltrate data outbound; `LLM_MODE=cloud` requires internet access from `mcp-server` for Anthropic API calls — document this trade-off and provide a compose override or profile for cloud mode that restores egress only for mcp-server while leaving indexer and Ollama isolated
- move `ANTHROPIC_API_KEY` from a plain environment variable to a Docker Compose secret for `LLM_MODE=cloud` deployments; environment variables are visible in `docker inspect mcp-server` output to anyone with Docker socket access, whereas Docker secrets are mounted as files and do not appear in inspect output; this follows the same pattern already applied to `bridge_pass` and keeps the credential-handling model consistent across all sensitive values
- add `-trimpath` to the Bridge Go build flags alongside the planned `-ldflags="-s -w"`; `-trimpath` removes all local file system paths embedded in the compiled binary (source file paths, module cache paths, build tool paths) so a `strings` or binary inspection of the shipped image reveals no host build environment details; without it, the bridge binary may embed the builder stage's directory layout, which could aid an attacker analyzing a captured image
- add SLSA build provenance attestation for the Bridge container image in `.github/workflows/bridge.yml` using `actions/attest-build-provenance`; Bridge handles decrypted email content and is built from patched third-party source — a signed attestation recording the source commit, build workflow, and runner identity lets operators verify that a running image was produced by the expected CI pipeline and not substituted; store the attestation alongside the image or as a workflow artifact
